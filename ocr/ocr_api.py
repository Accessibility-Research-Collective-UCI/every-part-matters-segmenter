...
def ocr_api(text, image_path):
    model.eval()

    conv = conversation_lib.conv_templates[args.conv_type].copy()
    conv.messages = []

    prompt = "<image>\nTell me the names of all the modules in the figure."
    ...
    text_output = text_output.split("[")[1].split("]")[0].split(",")
    text_output = [t.strip() for t in text_output]
    return text_output