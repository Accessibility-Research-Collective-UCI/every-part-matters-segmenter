...
def train(train_mode, data_mode):
    ...
    train_dataset, entity_to_idx, tag_to_idx = data_loader.train_data(
        train_config["BATCH_SIZE"], data_mode
    )
    print(entity_to_idx)
    print(tag_to_idx)
    configs = BertConfig.from_pretrained(
        "/data_share/model_hub/scibert", num_labels=len(tag_to_idx)
    )
    ...
    optimizer_grouped_parameters = [
        ...
        },
        {
            "params": [
                p
                for n, p in lstm_param_optimizer
                if not any(nd in n for nd in no_decay)
            ],
            "weight_decay": weight_decay,
        ...
        {
            "params": [
                p for n, p in lstm_param_optimizer if any(nd in n for nd in no_decay)
            ],
            "weight_decay": 0.0,
            "lr": lstm_learning_rate,
        },
    ...
    scheduler = transformers.get_linear_schedule_with_warmup(
        ...
        * total_batch_count
        * train_config["EPOCH"],
        num_training_steps=total_batch_count * train_config["EPOCH"],
    )
    ...
    if train_mode == "C":
        checkpoint = torch.load(
            "/home/llmtrainer/Multimodal/EPM/ner/bert_ner/checkpoint/bert_crf.pth"
        )
    ...
    for epoch in range(start_epoch + 1, train_config["EPOCH"]):
        ...
        for batch_num in tqdm(range(total_batch_count)):
            ...
            batch_train_data = train_dataset[
                batch_num
                * train_config["BATCH_SIZE"] : (batch_num + 1)
                * train_config["BATCH_SIZE"]
            ]
            batch_sentences = [line["sentence"] for line in batch_train_data]
            batch_entities = [line["entity"] for line in batch_train_data]

            encoding, input_ids, gold_tags = bpe_encode(
                batch_sentences, batch_entities, tokenizer, tag_to_idx
            )
            encoding = encoding.to(train_config["DEVICE"])
            attention_mask = torch.tensor(attention_masks(input_ids)).to(
                train_config["DEVICE"]
            )
            gold_tags = gold_tags.to(train_config["DEVICE"])

            outputs = model(
                input_ids=encoding, attention_mask=attention_mask, labels=gold_tags
            )
            loss = outputs[0]
            loss.backward()
        ...
        if f > f_max:
            ...
            print(
                "test-Entity: Recall - %f ; Precision - %f ; F-measure - %f" % (r, p, f)
            )
            state = {
            ...
            torch.save(
                model,
                "/home/llmtrainer/Multimodal/EPM/ner/bert_ner/checkpoint/bert_crf.pkl",
            )
...
def evaluation(model, mode, logger, tokenizer, data_mode, entity_to_idx, tag_to_idx):
    ...
    with torch.no_grad():
        ...
        for data in dataset:
            ...
            entity = data["entity"]
            encoding, input_ids, gold_tags = bpe_encode(
                [sentence], [entity], tokenizer, tag_to_idx
            )
            encoding = encoding.to(train_config["DEVICE"])
            attention_mask = torch.tensor(attention_masks(input_ids)).to(
                train_config["DEVICE"]
        ...
        logger.info(
            mode
            + "-Entity: Recall - %s ; Precision - %s ; F-measure - %s"
            % (str(recall), str(precision), str(f_measure))
        )
