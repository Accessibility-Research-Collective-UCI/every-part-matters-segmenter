...
def train_data(batch_size, data_mode):
    train_dataset, entity_to_idx, tag_to_idx = data_loader(
        os.path.join(data_dir, data_mode, "train.json")
    )
    last_batch_num = len(train_dataset) % batch_size
    if last_batch_num > 0:
        train_dataset += [
            train_dataset[ids] for ids in range(batch_size - last_batch_num)
        ]
