...
def parse_args(args):
    ...
    parser.add_argument(
        "--weight",
        default="/home/llmtrainer/Multimodal/EPM/checkpoint/pytorch_model_fu.bin",
        type=str,
    )
...
def attribute_api(image_path, name):
    ...
    output_dict = {"absolute_position": "", "relative_position": "", "function": ""}
    for prompt in prompt_list:
        ...
        if "absoulte position" in prompt:
            if "This module does not exist in the image." in text_output:
                ...
                try:
                    output_dict["absolute_position"] = (
                        text_output.split("in the image is ")[1]
                        .replace("</s>", "")
                        .strip()
                    )
                except:
